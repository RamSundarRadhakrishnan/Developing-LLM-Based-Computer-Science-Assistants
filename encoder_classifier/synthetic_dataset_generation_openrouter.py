...
CSV_FILE = "synthetic_dataset_openrouter.csv"
...
def send_request(prompt):
    ...
    while True:
        ...
        try:
            ...
            # Keep the full conversation locally; OpenRouter's OpenAI-compatible
            # chat API does not use Gemini's previous_interaction_id mechanism.
            conversation_messages.append({"role": "user", "content": prompt})
            conversation_messages.append({"role": "assistant", "content": output_text})
...
def start_new_conversation():
    ...
    # OpenRouter chat-completions conversations are maintained client-side.
    conversation_messages = [
...
def generate(prompt, expected_count):
    ...
    output_text = send_request(prompt)

    end_time = perf_counter()

    store_results(output_text)
